...
**原生例题的泛化判定**：
- 只要课件中出现「题面/任务/案例/场景/材料/数据/图表/代码/论述对象」以及「要求/问题/解法/步骤/答案/结论/点评」之一，就按原生例题处理，不局限于标题是否写了「例题」。
- 原生例题包括但不限于：计算题、推导题、证明题、图示分析题、表格/数据分析题、代码/伪代码题、实验设计题、案例分析题、材料阅读题、翻译/写作范例、课堂讨论题、选择判断讲评、综合应用题。
- 对不同学科，不预设固定题型；先观察课件反复出现的任务形态，再判断它训练的是「算」「推」「画」「读图」「读表」「写代码」「解释概念」「比较辨析」「分析材料」「组织论述」中的哪一种能力。
- 如果一页包含完整任务链条（给定条件 --&gt; 提问 --&gt; 解题过程/教师标注 --&gt; 结论），它的优先级高于只有单个概念定义或零散截图的页面。

**原生例题处理规则**：
1. 生成知识清单前，先扫描提取文本和图片页，搜索 `例`、`例题`、`练习`、`示例`、`Example`、`case`、`problem`、`question`、`exercise`、`quiz`、`题`、`案例`、`材料`、`讨论`、`思考`、`任务` 等关键词；同时识别编号题、选项、空格、问号、冒号后的任务、表格数据、代码块、公式推导、图中标注和教师手写/批注。
2. 每个高频考点至少保留 1 个最能代表该考点考法的课件原生例题；若原题很多，优先选择任务完整、步骤完整、覆盖核心方法、容易迁移成考试题、带关键图表/数据/代码/材料的例题。
3. 原生例题必须放在对应知识点之后，不要集中堆到文末。通用结构为：`方法总结 --&gt; 课件原题/原图/原材料 --&gt; 任务拆解 --&gt; 作答流程 --&gt; 易错点 --&gt; 可迁移套路`。
4. 如果例题图、表格、代码、材料页被图片提取拆碎，优先把 PDF/PPT 对应页面整页渲染成清晰图片，再嵌入 HTML。不要用破碎图块替代完整题面或上下文。
5. 对任何需要过程的题，都不能只给答案，必须写出可复用的作答流程：先判断题目要考什么能力，再列已知条件/材料信息，再选择方法或评价标准，最后给出步骤、结论和检查方式。
6. 对课件原题中的关键条件必须原样保留，包括数字、单位、范围、选项、图表标注、代码输入输出、材料原句、实验条件、评价维度、限定词和教师强调语，并解释这些条件如何影响解题或作答。
7. 章节测试中至少设置 2-5 道「课件原题变式」或「原材料/原图/原代码拆解题」。变式应保持原题训练的能力不变，只替换数据、情境、问法或干扰项；题干或 explanation 中根据需要嵌入原生例题图、表格、材料或代码片段。不能只出泛化概念题。

**生成后自查**：
- 如果知识清单没有出现任何「例题/课件原题/练习/示例」拆解，而原课件中存在例题，必须返工。
- 如果某个章节明显训练应用能力（计算、推导、证明、画图、读图、读表、编码、实验设计、案例分析、材料论述等），但知识清单只写概念定义，必须补充原生例题。
- 如果测试题里没有课件原题变式或原材料拆解题，必须补充至少 2 道。
